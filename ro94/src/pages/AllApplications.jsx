...
const AllApplications = () => {
  ...
  // --- MOCK DATA ---
  // Populated with realistic data based on your project scope [cite: 66, 73, 104]
  ...
  // --- DYNAMIC STATS ---
  // Calculates totals based on the 'applications' data above [cite: 173, 177]
  const stats = [
  ...
  // --- DROPDOWN OPTIONS ---
  // Matches options seen in your screenshots
  const filterOptions = {
  ...
  // Implements the color coding strategy from the proposal 
                ...
                <tbody className="divide-y divide-gray-100 text-sm text-slate-700">
                  {applications.map((app) => (
                    <tr key={app.id} className="hover:bg-slate-50/60 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold border border-transparent ${getStatusColor(app.docStatus)}`}>
                          {app.docStatus}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold border border-transparent ${getStatusColor(app.paymentStatus)}`}>
                          {app.paymentStatus}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                           ...
                           <span className="font-medium text-slate-700">{app.processStatus}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-slate-600 font-medium">{app.country}</td>
