...
const AllApplications = () => {
  ...
  return (
    <div className={`min-h-screen flex transition-colors duration-300 ${isDarkMode ? 'bg-[#09090b] text-zinc-100' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <main className="flex-1 min-h-screen overflow-auto pt-16 md:pt-20 font-sans relative">
        <Header />
        ...
        <div className="p-4 md:p-8 max-w-[1600px] mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className={`text-2xl md:text-3xl font-bold tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Applications</h1>
              <p className={`mt-1 text-sm md:text-base ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>Track and manage all job applications</p>
            </div>
            <button className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-medium flex items-center justify-center gap-2 shadow-sm transition-colors hover:shadow-blue-500/20">
              <Plus size={20} />
              New Application
            </button>
          </div>
              ...
              <div key={index} className={`p-5 rounded-2xl border shadow-sm flex flex-col justify-between h-32 transition-all duration-200
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h3 className={`text-2xl md:text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                      {stat.value}
                    </h3>
                  </div>
          ...
          {/* 3. FILTERS BAR */}
          <div className={`p-3 rounded-2xl border shadow-sm flex flex-col md:flex-row flex-wrap items-stretch md:items-center gap-3 transition-colors
          ...
          <div className="md:hidden space-y-4">
            {applications.map((app) => (
              <div key={app.id} className={`p-4 rounded-xl border shadow-sm ${isDarkMode ? 'bg-[#18181b] border-zinc-800' : 'bg-white border-gray-100'}`}>
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className={`font-semibold ${isDarkMode ? 'text-zinc-200' : 'text-slate-900'}`}>{app.name}</h4>
                      <p className={`text-xs ${isDarkMode ? 'text-zinc-500' : 'text-slate-500'}`}>{app.clientId}</p>
                    </div>
                  </div>
                  <ActionMenu isDarkMode={isDarkMode} onDelete={() => handleDelete(app.id)} />
                </div>

                <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                  <div>
                    <p className={`text-xs mb-1 ${isDarkMode ? 'text-zinc-500' : 'text-slate-500'}`}>Document</p>
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${getStatusColor(app.docStatus, isDarkMode)}`}>
                      {app.docStatus}
                    </span>
                  </div>
                  <div>
                    <p className={`text-xs mb-1 ${isDarkMode ? 'text-zinc-500' : 'text-slate-500'}`}>Payment</p>
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${getStatusColor(app.paymentStatus, isDarkMode)}`}>
                      {app.paymentStatus}
                    </span>
                  </div>
                  <div>
                    <p className={`text-xs mb-1 ${isDarkMode ? 'text-zinc-500' : 'text-slate-500'}`}>Process</p>
                    <div className={`flex items-center gap-2 ${isDarkMode ? 'text-zinc-300' : 'text-slate-700'}`}>
                  ...
                  <div>
                    <p className={`text-xs mb-1 ${isDarkMode ? 'text-zinc-500' : 'text-slate-500'}`}>Country</p>
                    <span className={isDarkMode ? 'text-zinc-300' : 'text-slate-700'}>{app.country}</span>
                  </div>
                </div>
                <div className={`pt-3 border-t flex items-center justify-between text-xs
                        ${isDarkMode ? 'border-zinc-800 text-zinc-500' : 'border-gray-100 text-slate-500'}`}>
                  <span>{app.docName}</span>
                  <div className="flex gap-2">
                    {/* Mobile Actions if needed inline */}
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className={`hidden md:block rounded-2xl border shadow-sm overflow-hidden transition-colors
            ${isDarkMode ? 'bg-[#18181b] border-zinc-800' : 'bg-white border-gray-100'}`}>
...
const ActionMenu = ({ isDarkMode, onDelete }) => (
  <div className="flex items-center gap-1">
    <ActionButton icon={Eye} isDarkMode={isDarkMode} color="blue" />
    <ActionButton icon={Edit2} isDarkMode={isDarkMode} color="emerald" />
    <ActionButton icon={Trash2} isDarkMode={isDarkMode} color="red" onClick={onDelete} />
  </div>
);
