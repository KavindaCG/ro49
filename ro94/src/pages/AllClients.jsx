        ...
        <div className="pt-16 md:pt-20 pb-8">
          ...
          <div className="max-w-[1600px] mx-auto px-4 md:px-8">
            ...
            <div className="mb-6 md:mb-8">
              ...
              <p className={`mt-1 text-sm md:text-base ${isDarkMode ? 'text-zinc-400' : 'text-gray-500'}`}>
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <h3 className={`text-2xl md:text-3xl font-bold mt-2 tracking-tight ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                        {stat.value}
                      </h3>
            ...
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6">
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-1">
                ...
                <div className="flex gap-2">
                  <button className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-colors
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <button className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-colors
                   ${isDarkMode ? 'bg-[#18181b] border-zinc-800 text-zinc-300 hover:bg-zinc-800' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50'}`}>
                  <Download size={18} />
                  <span>Export</span>
                </button>

                <Link to="/add-client" className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium transition-all shadow-sm">
                  <Plus size={18} />
            ...
            <div className="md:hidden space-y-4">
              {filtered.map((client) => (
                <div key={client.id} className={`p-4 rounded-xl border shadow-sm ${isDarkMode ? 'bg-[#18181b] border-zinc-800' : 'bg-white border-gray-200'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className={`text-xs ${isDarkMode ? 'text-zinc-500' : 'text-gray-500'}`}>{client.id}</p>
                      </div>
                    </div>
                    <div className="flex gap-1">
                      <button className={`p-2 rounded-lg ${isDarkMode ? 'text-zinc-400' : 'text-gray-400'}`}><Eye size={16} /></button>
                      <button className={`p-2 rounded-lg ${isDarkMode ? 'text-zinc-400' : 'text-gray-400'}`}><Edit size={16} /></button>
                    </div>
                  </div>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className={isDarkMode ? 'text-zinc-500' : 'text-gray-500'}>Email</span>
                      <span className={isDarkMode ? 'text-zinc-300' : 'text-gray-700'}>{client.email}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className={isDarkMode ? 'text-zinc-500' : 'text-gray-500'}>Broker</span>
                      <span className={isDarkMode ? 'text-zinc-300' : 'text-gray-700'}>{client.broker}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className={isDarkMode ? 'text-zinc-500' : 'text-gray-500'}>Status</span>
